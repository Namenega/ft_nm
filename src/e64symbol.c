...
static void	swap_sym(t_64Symbol *a, t_64Symbol *b) {
...
static int	symbol_cmp(const t_64Symbol *a, const t_64Symbol *b) {
...
static void	sort64Symbols(t_64Symbol *array, int left, int right) {
	...
	sort64Symbols(array, left, j - 1);
	sort64Symbols(array, j + 1, right);
}
...
static char	getELF64SymbolType(Elf64_Sym symbol, char type, Elf64_Shdr *shdrs) {
...
void		symbol64Handling(void *elf_data, t_Elf64data edata) {
	...
	if (!(symbols = malloc(sizeof(t_64Symbol) * sym_count)))
		return ;
	...
	// sort symbols
	sort64Symbols(symbols, 0, valid_count - 1);
	...
	for (int i = 0; i < valid_count; i++) {
		...
		char type = '?';
		type = getELF64SymbolType(sym, type, edata.shdr);
